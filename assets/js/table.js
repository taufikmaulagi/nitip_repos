	...
	$.fn.serverside = function (options) {
		...
		this.each(function () {
			...
			if ($t.find('th[data-content]').length > 0) {
				...
				$t.find('th[data-content]').each(function () {
					...
					if (typeof $(this).attr('data-content') != 'undefined') {
						if (typeof $(this).attr('data-custom') == 'undefined') {
					...
					if (typeof $(this).attr('data-filter') == 'undefined' && typeof $(this).attr('data-alias') != 'undefined' && $(this).attr('data-field') != $t.attr('data-table') + '.id') {
						...
						if (typeof $(this).attr('data-type') != 'undefined' && $(this).attr('data-type') == 'boolean') {
							var boolValue = [1, 2];
							if (typeof $(this).attr('data-boolean-value') != 'undefined') {
								var spValue = $(this).attr('data-boolean-value').split(',');
								boolValue = spValue;
							}
							if (typeof $(this).attr('data-boolean-text') != 'undefined') {
								var spText = $(this).attr('data-boolean-text').split(',');
								filter += '<th><select class="custom-select select-boolean" data-filter="' + f + '" style="min-width:90px"><option value=""></option>';
								$.each(spText, function(i,v){
									filter += '<option value="' + boolValue[i] + '">' + spText[i] + '</option>';
								})
								filter +='</select></th>';
							} else {
								filter += '<th><select class="custom-select select-boolean" data-filter="' + f + '" style="min-width:90px"><option value=""></option><option value="' + boolValue[0] + '">TRUE</option><option value="' + boolValue[1] + '">FALSE</option></select></th>';
							}
			...
			if (typeof $('[data-serverside]').attr('data-fixed') == 'undefined') {
				setTimeout(function () {
					$('.footer-serverside').css({
						'width': $('[data-serverside]').outerWidth()
					});
				}, 500);
			}
		});
		...
		function getData(e, refresh, refix) {
			...
			if (p_data || refresh) {
				...
				// konten = '<tr class="not-hover"><td colspan="' + colspan + '" class="result-error"><img src="' + base_url + 'assets/images/no-data.svg" width="30%"><p>&nbsp;</p><h3>' + lang.tidak_ditemukan_kecocokan_data_dari_pencarian + '</h3></td></tr>';
							...
							if (parseInt(res.jmlShow) <= 0) {
								if (jmlLimit == 0) {
									...
									if (typeof res.jmlFilter == 'undefined') {
										konten = '<tr class="not-hover"><td colspan="' + colspan + '" class="result-error"><img src="' + base_url + 'assets/images/no-data.svg" width="200px"><h3>' + lang.tidak_ada_data + '</h3></td></tr>';
										$('.footer-serverside [data-info]').text(lang.tidak_ada_data);

									} else {
										konten = '<tr class="not-hover"><td colspan="' + colspan + '" class="result-error"><img src="' + base_url + 'assets/images/no-data.svg" width="200px"><h3>' + lang.tidak_ditemukan_kecocokan_data_dari_pencarian + '</h3></td></tr>';
										$('.footer-serverside [data-info]').text(lang.tidak_ada_data_yang_cocok);
									}
								} else {
									$('[data-serverside]').show();
									$('#img').html('');
									proccess = false;
										...
										e.children('thead').children('tr').first().children().each(function () {
											...
											if (typeof $(this).attr('data-alias') != 'undefined') {
												if (typeof res.data[i][$(this).attr('data-alias')] != 'undefined') {
														...
														if (typeof $(this).attr('data-replace') != 'undefined') {
															...
															if (typeof $(this).attr('data-type') != 'undefined' && $(this).attr('data-type') == 'boolean') {
																...
																if (typeof $(this).attr('data-icon') == 'undefined') {
																	...
																	// $.each(boolText, function(i,v){
																	// 	if(boolValue[i] == i)
																	// 	string = '<span class="badge badge-success">' + v + '</span>';
																	// 	str_label = 'TRUE';
																	// });
																	if(boolValue.length > 2){

																		if (string == boolValue[0]) {
																			string = '<span class="badge badge-success">' + boolText[0] + '</span>';
																			str_label = 'TRUE';
																		} else if (string == boolValue[1]) {
																			string = '<span class="badge badge-danger">' + boolText[1] + '</span>';
																			str_label = 'FALSE';
																		} else if (string == boolValue[2]) {
																			string = '<span class="badge badge-warning">' + boolText[2] + '</span>';
																			str_label = 'WAITING';
																		} else if(boolValue[3] != undefined && string == boolValue[3]) {
																			string = '<span class="badge badge-secondary">' + boolText[3] + '</span>';
																			str_label = 'OTHER';
																		}
																	} else {
																		if (string == boolValue[0]) {
																...
																if (e.find('input[data-filter="' + xf + '"]').length == 1) {
																	...
																	if (settings.highlight == true) {
																		...
																		if (string.length > 0) {
																			str = string.replace(reg, function (st) {
																				return '<span class="highlight">' + st + '</span>'
																			});
																		}
																		string = str;
																	}
													...
													konten += '<td' + cls + ' class="text-center">???</td>';
												}
											} else {
												if (typeof $(this).attr('data-action') == 'undefined') {
													...
													if (res.accessEdit == '1' || res.accessDelete == '1' || res.accessView == '1') {
														...
														if (res.accessView == '1') {
															...
															if (res.linkView == '') {
																konten += '<button type="button" class="btn btn-sky btn-act-view" data-key="view" data-id="' + res.data[i][$('[data-serverside]').attr('data-table') + '_id'] + '" title="' + lang.detil + '"><i class="fa-search"></i></button>' + "\n";
															} else {
																konten += '<a href="' + res.linkView + encode_id + '" class="btn btn-sky" data-key="view" title="' + lang.detil + '"><i class="fa-search"></i></a>' + "\n";
															}
														}
														if (res.accessEdit == '1' && (typeof $('[data-serverside] th[data-action]').attr('data-edit') == 'undefined' || (typeof $('[data-serverside] th[data-action]').attr('data-edit') !== 'undefined' && typeof $('[data-serverside] th[data-action]').attr('data-edit') == 'true'))) {
															...
															if (res.linkEdit == '') {
																konten += '<button type="button" class="btn btn-sunny btn-input" data-key="edit" data-id="' + res.data[i][$('[data-serverside]').attr('data-table') + '_id'] + '" title="' + lang.ubah + '"><i class="fa-edit"></i></button>' + "\n";
															} else {
																konten += '<a href="' + res.linkEdit + encode_id + '" class="btn btn-sunny" data-key="edit" title="' + lang.ubah + '"><i class="fa-edit"></i></a>' + "\n";
															}
														}
														if (res.accessDelete == '1') {
															...
															if (res.linkDelete == '') {
																konten += '<button type="button" class="btn btn-hot btn-delete" data-key="delete" data-id="' + res.data[i][$('[data-serverside]').attr('data-table') + '_id'] + '" title="' + lang.hapus + '"><i class="fa-trash-alt"></i></button>';
															} else {
																konten += '<a href="' + res.linkDelete + encode_id + '" class="btn btn-hot" data-key="delete" title="' + lang.hapus + '"><i class="fa-trash"></i></a>';
															}
														}
													} else {
														konten += '&nbsp;';
		...
		if ($('table[data-fixed="true"]').length > 0 && $('table[data-fixed="true"] tbody tr').length > 0) {
			if (has_fixed == false) {
				...
				$('[data-fixed="true"] thead th').each(function (i, j) {
					...
					$(this).css({
						'min-width': Math.round(arr[i]),
						'max-width': Math.round(arr[i])
					})
				});
				...
				has_fixed = true;
				$('.fixed-table.header table').css({
					'margin-left': left_fixed
				});
