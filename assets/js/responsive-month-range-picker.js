(function ($) {
    $.fn.responsiveMonthRange = function (options) {
        ...
        if (_settings.button.fiscalYtd.show) {
            content += '<button class="btn btn-sky rmrp-fiscal-ytd">'+_settings.button.fiscalYtd.label+'</button>';
        }
        if (_settings.button.ytd.show) {
            content += '<button class="btn btn-sky rmrp-ytd">'+_settings.button.ytd.label+'</button>';
        }
        if (_settings.button.previousFY.show) {
            content += '<button class="btn btn-sky rmrp-prev-fiscal">'+_settings.button.previousFY.label+'</button>';
        }
        if (_settings.button.previousYear.show) {
            content += '<button class="btn btn-sky rmrp-prev-year">'+_settings.button.previousYear.label+'</button>';
        }
        content += '<button class="btn btn-sky btn-apply">'+_settings.button.apply.label+'</button>';
        content += '</div>';
        content += '</div>';
