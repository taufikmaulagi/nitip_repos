    ...
    <div class="row pt-4">
        <div class="col-sm-9">
            ...
            <div class="row mt-4">
                ...
                <div class="col-sm-6">
                    <h5 style="font-weight: 700"> Use Confirm - Bulan November </h5>
                    <div class="row">
                        <div class="col-sm-12">
            ...
            <div class="row mt-4 ml-2">
                <h5 style="font-weight: 700"> Sales Achievment - Bulan <?=strftime('%B', strtotime(date('Y-m-d')))?> </h5>
                <?php
                    echo $html_data_sales
                ?>
            </div>
            <div class="row mt-4 ml-2">
                <h5 style="font-weight: 700"> Sales Performance By Month </h5>
                <div class="col-sm-12">
                    <canvas id="chartAchievment" height="300" width=890"></canvas>
                </div>
            </div>
...
<script>
   ...
   let tahun = [];
    ...
    let achievment = JSON.parse('<?=json_encode($achievment)?>')
    let growth = JSON.parse('<?=json_encode($growth)?>')
    let target_data = JSON.parse('<?=json_encode($target_data)?>')
    let actual_data = JSON.parse('<?=json_encode($actual_data)?>')
    ...
    const ctx4 = document.getElementById('chartAchievment').getContext('2d')
