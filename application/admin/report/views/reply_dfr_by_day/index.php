	...
	<!-- <div class="main-container"> -->
		<?php
			if(get('tahun') != '' && get('bulan') != '' && get('team_group') != ''){ ?>
				<table class="table-app table-bordered">
					<thead>
						<tr>
							...
							<?php
								...
								$rm = get_data('history_organogram_detail a', [
									'select' => 'a.n_rm, a.nama_rm, c.id as id_user',
									'join' => [
										'history_organogram b on a.id_history_organogram = b.id',
										'tbl_user c on a.n_rm = c.username'
									],
									'where_in' => [
								...
								foreach($rm as $vrm){
									...
									echo '<th class="bg-secondary text-white" colspan="'.(count($produk_group)+1).'"	>'.$vrm['nama_rm'].' (RM) </th>';
									array_push($data_user, [
										'type' => 'RM',
										'nip' => $vrm['n_rm'],
										'id' => $vrm['id_user'],
					...
					<tbody>
						<?php
							...
							$feedback = get_data('trxdfr_'.get('tahun').'_'.str_pad(get('bulan'), 2, '0', STR_PAD_LEFT).' b',[
								'select' => 'count(*) as jumlah, a.user, date(a.cat) as tanggal, produk_grup',
								'group_by' => 'a.user, date(cat), b.produk_grup',
								'where_in' => [
