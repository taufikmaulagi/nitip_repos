...
class Obrolan extends BE_Controller {
	...
	function data() {
		...
		if($access['access_additional']) {
			$config['button'][] = button_serverside('btn-sky','btn-export',['fa-download',lang('ekspor'),true],'act-export');
		}
		if($access['access_edit']) {
			$config['access_edit'] = false;
			$config['button'][] = button_serverside('btn-sunny','btn-input',['fa-edit',lang('ubah'),true],'act-edit',[
				'is_group'	=> 1
			]);
		}
