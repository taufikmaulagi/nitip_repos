...
<div class="content-header">
	<div class="main-container position-relative">
		...
		<?php if($menu_access['access_edit']) { ?>
			<button type="button" class="btn btn-fresh btn-sm btn-sort"><i class="fa-align-right"></i><?php echo lang('atur_posisi'); ?></button>
		<?php } ?>
		<?php if($menu_access['access_input'] && user('id_group') == 1 && ENVIRONMENT == 'development') { ?>
			<button type="button" class="btn btn-sky btn-sm btn-input" data-id="0"><i class="fa-plus"></i><?php echo lang('tambah'); ?></button>
		<?php } ?>
		</div>
...
<?php 
	...
	modal_open('modal-sort',lang('atur_posisi'),'modal-lg','modal-info');
		modal_body();
		modal_footer();
			echo '<form><button type="submit" class="btn btn-fresh" id="save-posisi">'.lang('simpan').'</button></form>';
	modal_close();
?>
