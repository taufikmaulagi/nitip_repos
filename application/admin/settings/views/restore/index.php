<div class="content-header">
	<div class="main-container position-relative">
		...
		<div class="float-right">
			<button type="button" class="btn btn-sky btn-sm btn-act-import"><i class="fa-upload"></i><?php echo lang('unggah'); ?></button>
		</div>
		<?php } ?>
	...
	table_open('',true);
		...
		tbody();
			if(count($backup) > 0) {
				foreach($backup as $b) {
					$button = '';
					$button .= '<button type="button" class="btn btn-sunny btn-restore" data-key="restore" data-id="'.$b.'" title="'.lang('restore').'"><i class="fa-redo"></i></button>';
					tr();
						td($b);
						td($button,'button');
				}
