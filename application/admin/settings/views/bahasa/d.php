...
<?php
	table_open('',true);
		...
		tbody();
            tr();
                td('_js');
                td('<button type="button" class="btn btn-sunny btn-edit-lang" data-key="edit" data-id="_js" title="'.lang('ubah').'"><i class="fa-edit"></i></button>','button');
            foreach($file as $b) {
                if(strpos($b,'.json') != false) {
                    tr();
                        td(str_replace('.json','',$b));
                        td('<button type="button" class="btn btn-sunny btn-edit-lang" data-key="edit" data-id="'.str_replace('.json','',$b).'" title="'.lang('ubah').'"><i class="fa-edit"></i></button>','button');
                }
			}
    table_close();
?>
