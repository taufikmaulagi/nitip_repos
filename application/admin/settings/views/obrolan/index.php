		...
		form_open(base_url('settings/obrolan/save'),'post','form');
			...
			<div class="form-group row">
				...
				<div class="col-sm-2 col-3">
					<button type="button" class="btn btn-block btn-fresh btn-icon-only btn-add-anggota"><i class="fa-plus"></i></button>
				</div>
			</div>
...
function add_row_anggota() {
	konten = '<div class="form-group row">'
			...
			+ '</div>'
			+ '<div class="col-sm-2 col-3">'
			+ '<button type="button" class="btn btn-block btn-hot btn-icon-only btn-remove-anggota"><i class="fa-times"></i></button>'
			+ '</div>'
			+ '</div>';
...
$(document).on('click','.btn-input',function(){
	...
	if($(this).data('id') == 0) {
		...
		$('#modal-form [type="submit"]').html('<i class="fa-check mr-2"></i>' . lang.simpan);
	} else {
		mtitle = lang.ubah;
