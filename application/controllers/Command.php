								...
								if(!file_exists($dir_module.$module->target.'/controllers/'.$nama_menu.'.php') || (count($c) == 4 && $c[3] == '--force') || (count($c) == 6 && $c[5] == '--force') ) {
									...
									if(count($child) > 0) {
										...
										if(!isset($c[3]) || (isset($c[3]) && $c[3] == '--force')) {
											...
											$html 	.= add_tab($i) . '<div class="header-info d-none d-sm-block">' . PHP_EOL; $i++;
											...
											if(isset($c[4]) && $c[3] == '-crud' && $c[4]) {
												if(table_exists($c[4])) {
													...
													$alias		= [];
													foreach($field as $f) {
														if($f->name != 'create_by' && $f->name != 'create_at' && $f->name != 'update_by' && $f->name != 'update_at' && $f->name != 'cat' && $f->name != 'uat' && $f->name != 'dat') {
