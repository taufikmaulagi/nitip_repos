...
function breadcrumb($last='',$multiple_child=false) {
    ...
    if(isset($cur_menu->id)) {
        ...
        if($cur_menu->level4) {
            $menu_l1    = get_data('tbl_menu','id',$cur_menu->level3)->row();
            if(isset($menu_l3->id)) {
                $br[]   = array(
	...
	if($delete || $active || $inactive || $export || $import || (isset($additional) && $additional)) {
		...
		if($input) {
			if($link_add && !is_array($link_add)) {
				$a_button .= '<a href="'.$link_add.'" class="btn btn-sky btn-sm"><i class="fa-plus"></i>'.lang('tambah').'</a>' . PHP_EOL;
			} else {
				$a_button .= '<button type="button" class="btn btn-sky btn-sm btn-input" data-id="0"><i class="fa-plus"></i>'.lang('tambah').'</button>' . PHP_EOL;
			}
		}
		$a_button .= '<div class="btn-group" role="group">' . PHP_EOL;
		$a_button .= '<button id="btnAccess" type="button" class="btn btn-sm btn-sky dropdown-toggle'.$add_class.'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
...
function modal_open($id='',$title="Modal",$modal_tipe='',$attr='',$header_class='') {
	$CI = get_instance();
	...
	$html .= '<div class="modal-header '.$header_class.'">' . PHP_EOL;
...
function label($label='',$cls='',$attr='') {
	...
	$html .= '<h4 class="col-form-label col-12 '.$cls.'" '.$attr.' style="font-weight:900; text-transform:uppercase">'.$label.'</h4>' . PHP_EOL;
	$html .= '</div>' . PHP_EOL;
	echo $html;
}
function input($tipe='',$label='',$nama='',$validation='',$value='',$attr='',$label_group='') {
	...
	if($tipe == 'hidden') {
		...
		if($tipe == 'icon') {
			...
			if(!$CI->session->userdata('g_inputgroup')) {
				$html .= '<input type="hidden" name="'.$nama.'" id="'.$_id.'" autocomplete="off" class="jq-range" data-validation="'.$validation.'" value="'.$value.'"'.$attr.'>' . PHP_EOL;
			}
				...
				foreach($x_opt_attr as $c_attr) {
					...
					if(count($x_c_attr) == 2) {
						if(isset($d[$x_c_attr[1]])) {
							$_opt_attr .= ' '.$x_c_attr[0].'="'.$d[$x_c_attr[1]].'"';
						} else {
...
function fileupload($label='',$nama='',$validation='',$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<div class="input-group-append">' . PHP_EOL;
		$html .= '<button class="btn btn-fresh btn-file" type="button">'.lang('unggah').'</button>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
...
function imageupload2($label='',$nama='',$validation='',$width=256,$height=256,$tipe='',$value='',$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<div class="input-group-append">' . PHP_EOL;
		$html .= '<button class="btn btn-sky btn-view-imageupload" type="button"><span class="fa-eye"></span></button>' . PHP_EOL;
		$html .= '<button class="btn btn-fresh btn-image" type="button">'.lang('unggah').'</button>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
...
function form_button($submit='Submit',$reset='Reset') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<button type="submit" class="btn btn-sky">'.$submit.'</button>' . PHP_EOL;
		if($reset) {
			$html .= '<button type="reset" class="btn btn-fresh">'.$reset.'</button>' . PHP_EOL;
		}
		$html .= '</div>' . PHP_EOL;
		$html .= '</div>' . PHP_EOL;
	}
