...
function field_exists($table="",$column="") {
    ...
    $db_group   = $CI->load->database($db_active,TRUE);
    if($column) {
        return $db_group->field_exists($column, $table);
    } else
        return false;
}
...
function get_data($table="",$attr=array(),$column=''){
    $CI         = get_instance();
    // $CI->db->query("ONLY_FULL_GROUP_BY from mysql console mysql > SET GLOBAL sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))");
    $db_active  = $CI->session->userdata('db_active');
    $db_group   = $db_active ? $CI->load->database($db_active,TRUE) : $CI->db;
    if(is_array($attr)) {
